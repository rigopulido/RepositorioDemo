...
--- | --- | --- | ---
recordsField | String | 'features' | A pointer to the field in the input data that contains the records to be visualized.  Use _null_ when the data being passed in is the set of records to be visualized.  Use dot notation to specify child properties (e.g. _data.election.resultsByState_), see note below.
...
deriveProperties | Function | null | A function that takes a record as input and returns a new record derived from that input record.  This is useful for styling features based on properties derived from some combination of properties in the base record.
getIndexKey | Function | null | A function that takes a location and record as input and returns a string that will be used to uniquely identify a layer created from a given record.  Supplying this function ensures that layers won't be removed and re-added when you change data or call setDisplayOptions/setDisplayOption.
getTrack | Function | null | A function that takes a layer, location, and reference to an existing track layer and returns an updated track layer.  This is used in conjunction with getIndexKey to highlight the movement of layers.
